#!/usr/bin/env python3
"""老挝语↔中文 翻译 App (Kivy + Flask + 离线NLLB模型)

架构：
- 内嵌 Flask Web 服务 (localhost:5001)
- Kivy WebView 加载本地页面
- 支持百度API在线翻译 + NLLB离线翻译
"""
...
textarea{width:100%;min-height:150px;padding:14px;border:none;background:transparent;
         color:#e2e8f0;font-size:1rem;line-height:1.5;resize:vertical;outline:none;font-family:inherit}
textarea::placeholder{color:#475569}
.out{padding:14px;min-height:150px;font-size:1rem;line-height:1.5;white-space:pre-wrap;color:#e2e8f0}
...
<div class="container">
  ...
  <div class="panel">
    ...
    <textarea id="src" placeholder="输入要翻译的文字..." oninput="uc()"></textarea>
  </div>
  <div class="panel">
...
function ul(){const n={lo:'老挝语',zh:'中文'};document.getElementById('sl').textContent=n[f]||f;
  document.getElementById('tl').textContent=n[t]||t;}
...
async function openS(){try{let r=await fetch('/config');let c=await r.json();
  document.getElementById('ai').value=c.baidu_app_id||'';document.getElementById('sk').value=c.baidu_secret_key||'';}catch(e){}
...
document.getElementById('src').addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter'){e.preventDefault();go();}});
ckS();ul();
</script>
</body></html>'''
...
class TranslatorApp(App):
    ...
    def _start_server(self):
        try:
            print('[翻译] 启动 Flask 服务...')
            flask_app.run(host='127.0.0.1', port=5001, debug=False, use_reloader=False)
        except Exception as e:
            print(f'[翻译] 服务启动失败: {e}')
            Clock.schedule_once(lambda dt: setattr(self.root.ids.status, 'text', f'启动失败: {e}'), 0)
